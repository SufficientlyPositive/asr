...
pub trait PhonemeRetriever<'a> {
    fn find_likelihood(&self, sample: Vec<Amplitude32>) -> Result<Vec<(&'a str, Probability64)>>; // assumes all samples are normalised to range (-1,1), with 1 being max elem, may revisit this later
    
    fn find_sorted_likelihood(&self, sample: Vec<Amplitude32>) -> Result<Vec<(&'a str, Probability64)>> { // somewhat unsafe? assumes all f32 values != NaN (they should be in the range 0-1 anyways...).
